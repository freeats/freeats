...
export default class extends Controller {
  ...
  #createPopoverWithButton(element) {
    ...
    element.addEventListener('show.bs.popover', () => {
      const shownPopover = document.querySelectorAll('[data-copy-to-clipboard-link-with-popover-value="true"]');
  ...
  async #inviteLink() {
    ...
    if (this.dataset.clipboardPlainText === undefined) {
      const response = await fetch(`/shorten_link.json?link=${encodeURIComponent(longLink)}&slashtag=${slashtag}`);
      if (response.ok) {
    ...
    if (isLink) {
      window.location.href = `${linkWithMailTo}body=${encodeURIComponent(this.dataset.clipboardPlainText)}`;
    }
