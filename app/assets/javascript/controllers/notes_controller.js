...
import { storeFormDraft, retrieveFormDraft, clearFormDraft } from '../src/ats/draft_storage';
  ...
  handleLinkToNote() {
    ...
    if (linkedNoteDomId.match(/#note-\d+/)) {
      ...
      if (isReply) {
        setTimeout(
          () => {
            const scrollOffset = $(window.location.hash).offset().top - $('nav.navbar.header').innerHeight();
            $(window).scrollTop(scrollOffset);
